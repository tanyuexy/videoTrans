...
function detectParagraphs(text) {
  ...
  if (text.includes(paragraphSeparators)) {
    paragraphs = text.split(paragraphSeparators)
      .map(p => p.trim())
      .filter(p => p.length > 0);
  }
...
function extractPCMFromWAV(wavBuffer) {
  ...
  while (dataOffset < wavBuffer.length - 8) {
    ...
    if (chunkId === 'data') {
      const pcmData = wavBuffer.subarray(dataOffset + 8, dataOffset + 8 + chunkSize);
      return pcmData;
    }
 ...
 */
function generateSilencePCM(durationSeconds, sampleRate = 24000, channels = 1, bitDepth = 16) {
 ...
 */
function concatenatePCMWithSilence(pcmBuffers, silenceDurations = [], sampleRate = 24000, channels = 1, bitDepth = 16) {
  ...
  for (let i = 0; i < pcmBuffers.length; i++) {
    ...
    if (i < pcmBuffers.length - 1) {
      ...
      if (silenceDuration > 0) {
        const silencePCM = generateSilencePCM(silenceDuration, sampleRate, channels, bitDepth);
 ...
 */
async function generateSpeech(text, voiceName = 'Kore', outputPath, paragraphInterval = 0) {
  try {
    if (!apiKey) {
    ...
    const concatenatedPCM = concatenatePCMWithSilence(pcmBuffers, silenceDurations);
