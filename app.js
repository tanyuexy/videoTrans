...
import {
  ...
  transcribeBothWithGemini,
  validateGeminiConfig
} from "./utils/geminiTranscriber.js";
import {
  translateText,
  getSupportedLanguages,
  generateShortTitle
} from "./utils/geminiTranslator.js";
import {
  generateSpeech,
  validateTTSConfig,
  getVoiceOptions,
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    cb(
      new Error(
        "只支持视频/音频文件格式: mp4, avi, mov, wmv, flv, webm, mkv, mp3, m4a, aac, ogg, flac, wav"
      )
    );
  }
});
...
app.post("/api/transcribe", upload.single("video"), async (req, res) => {
  try {
    ...
    try {
      ...
      const { original: originalTranscription, chinese: chineseTranscription } =
        await transcribeBothWithGemini(audioPath);
...
app.get("/api/supported-languages", (req, res) => {
  try {
    ...
    if (model === "aliyun" || model === "ali") {
      ...
      languages = Object.entries(LANGUAGE_CODE_TO_FULL_NAME).map(
        ([code, name]) => ({ code, name })
      );
    } else {
...
app.get("/api/voice-sample/:voiceName", (req, res) => {
  try {
    ...
    if (model === "aliyun" || model === "ali") {
      ...
      const aliyunVoices = getAliyunVoiceOptions();
      const found = aliyunVoices.find(
        (v) => v.name === voiceName || v.voice === voiceName
      );
      if (!found || !found.auditionAudio) {
        return res
          .status(404)
          .json({ success: false, error: "未找到阿里云语音样本" });
      }
      ...
      res.setHeader(
        "Content-Disposition",
        `inline; filename="${sampleFileName}"`
      );
...
app.post("/api/generate-speech", async (req, res) => {
  try {
    ...
    // 验证段落间隔参数
    const intervalSeconds = paragraphInterval
      ? parseFloat(paragraphInterval)
      : 0;
    ...
    if (skipTranslate === true) {
      ...
      try {
        ...
        console.log(
          `翻译完成: ${translatedText.substring(0, 100)}${
    ...
    const existingFiles = fs
      ...
      .filter(
        (file) =>
          file.startsWith(`${dateStr}_${targetLanguage}_`) &&
          file.endsWith(".wav")
      );
    ...
    try {
      ...
      if (usedModel === "aliyun" || usedModel === "ali") {
        ...
        const audioBuffer = await callAliyunTTSOnce(
          translatedText,
          voiceName,
          languageType
        );
      ...
      return res.status(500).json({
        ...
        error: "语音生成失败",
        details:
          speechError && speechError.message ? speechError.message : speechError
      });
    }
    ...
    // 5. 生成用于展示和下载的文件命名（可选，使用内容摘要）
    let displayFileName = audioFileName;
    if (useContentFileName && namingText && namingText.trim()) {
      try {
        const shortTitle = await generateShortTitle(namingText);
        if (shortTitle && shortTitle.trim()) {
          const dotIndex = audioFileName.lastIndexOf(".");
          let baseName =
            dotIndex > 0 ? audioFileName.slice(0, dotIndex) : audioFileName;
          const ext = dotIndex > 0 ? audioFileName.slice(dotIndex) : ".wav";
          // 去掉序号部分（格式：日期_语言_序号 -> 日期_语言）
          baseName = baseName.replace(/_\d+$/, "");
          let safeTitle = shortTitle.replace(/\s+/g, " ").trim();
          safeTitle = safeTitle.replace(/[\\\/:*?"<>|]/g, "");
          if (safeTitle) {
            displayFileName = `${baseName}_${safeTitle}${ext}`;
          }
        }
      } catch (e) {
        console.error("生成摘要命名失败，将使用默认文件名:", e);
      }
    }
